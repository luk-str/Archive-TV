...
function generateFetchUrl() {
    ...
    return `https://archive.org/advancedsearch.php?q=collection%3A%28moviesandfilms%29+AND+mediatype%3A%28movies%29&fl%5B%5D=identifier&fl%5B%5D=title&fl%5B%5D=date&sort%5B%5D=__random+desc&sort%5B%5D=&sort%5B%5D=&rows=10&page=${randomPageNumber}&output=json`
}
...
fetch(
  ...
  .then((data) => {
    ...
    const filmUrl = `https://archive.org/embed/${randomFilm.identifier}&autoplay=0`;
